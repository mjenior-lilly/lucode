...
def test_returns_none_when_npm_missing(monkeypatch):
    monkeypatch.setattr("lucode.agents.updates.shutil.which", lambda _: None)

    assert available_npm_package_update("opencode-ai") is None


def test_returns_none_when_package_is_current(monkeypatch):
    monkeypatch.setattr("lucode.agents.updates.shutil.which", lambda _: "/usr/bin/npm")
    monkeypatch.setattr(
        "lucode.agents.updates.subprocess.run",
...
def test_returns_current_and_latest_when_outdated(monkeypatch):
    ...
    monkeypatch.setattr("lucode.agents.updates.subprocess.run", fake_run)
