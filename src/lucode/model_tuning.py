"""Packaged per-model tuning for Pi and OpenCode.
...
Discovery returns bare model ids, so without this tuning an agent runs with no
...
Two rules govern how the tuning is applied, both enforced by callers in
``agents/pi.py`` and ``agents/opencode.py``:

- *Membership* is decided by discovery or by a workspace-managed inventory.
  This module never adds a model an agent was not told to serve.
- *Tuning* is layered underneath the user's own config. An existing entry in
...
# credential. Stripped on load: the packaged files are a tuning reference, and
...
@lru_cache(maxsize=1)
def _pi_tuning() -> dict[str, dict[str, dict[str, Any]]]:
    """Pi tuning as ``{provider: {model_id: tuning}}``, credentials stripped."""
    ...
    for provider, config in providers.items():
        ...
        if entries:
            tuned[provider] = entries
    return tuned


@lru_cache(maxsize=1)
def _opencode_tuning() -> dict[str, dict[str, dict[str, Any]]]:
    """OpenCode tuning as ``{provider: {model_id: tuning}}``, credentials stripped.
    ...
    so it is tuning rather than a credential.
    """
...
def pi_model_tuning(provider: str, model_id: str) -> dict[str, Any]:
    """Packaged Pi tuning for one model, or {} when none is known.

    The returned dict excludes ``id`` so callers can merge it onto an entry they
    already keyed by id.
    """
    tuning = _pi_tuning().get(provider, {}).get(model_id)
    if not tuning:
        return {}
    return {k: deepcopy(v) for k, v in tuning.items() if k != "id"}


def opencode_model_tuning(provider: str, model_id: str) -> dict[str, Any]:
    """Packaged OpenCode tuning for one model, or {} when none is known."""
    return deepcopy(_opencode_tuning().get(provider, {}).get(model_id, {}))


def pi_tuned_model_ids(provider: str) -> tuple[str, ...]:
    """Model ids with packaged Pi tuning, in packaged order."""
    return tuple(_pi_tuning().get(provider, {}))


def opencode_tuned_model_ids(provider: str) -> tuple[str, ...]:
    """Model ids with packaged OpenCode tuning, in packaged order."""
    return tuple(_opencode_tuning().get(provider, {}))
