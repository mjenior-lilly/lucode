...
`lucode` is a Python CLI that configures and launches coding agents through Databricks AI Gateway.

The package code lives in `src/lucode/`.
Tests live in `tests/`.
...
- Run e2e tests with `lucode_TEST_WORKSPACE=<db_workspace_url> uv run pytest tests/test_e2e.py -v`.
- Run lint with `uv run ruff check .`.
- Run the CLI from the current checkout with `uv run lucode ...`.
- Reinstall the local checkout as the `lucode` tool with `uv tool install --reinstall .`.

## Development

- Use Python 3.12+.
- Keep changes scoped to the requested behavior.
- Follow the existing module boundaries: CLI orchestration in `cli.py`, agent-specific behavior in `agents/<name>.py`, shared agent dispatch in `agents/__init__.py`, skill download (UC fetch client + on-disk writer + download orchestration) in `skills_download.py`, MCP-connection state and its interactive resource picker in `mcp.py`, and shared presentation helpers in `ui.py`. Databricks integration is split by concern: HTTP/diagnostics in `databricks/transport.py`, CLI authentication and command builders in `databricks/auth.py`, model discovery and agent base URLs in `databricks/models.py`, administration and managed config in `databricks/managed.py`, MCP/UC resource discovery in `databricks/mcp_discovery.py`, and warehouse/query behavior in `databricks/sql.py`. Import from the owning submodule; `lucode.databricks` intentionally has no aggregate exports. Skill download persists no disk state — it writes files to `--path` (or the home dir) and registers only the schema-less skills MCP connection. `lucode configure skills` with no `--location` (or `--mcp` with no `--location`) registers that schema-less connection without downloading anything.
